...
describe('decisions', () => {
  ...
  it('honours truncateHeadChars, including a zero head', () => {
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[claude-compact-openrouter truncated ${total - 50} chars of this tool result; re-run the tool if needed]`,
    );
    ...
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[claude-compact-openrouter truncated ${total} chars of this tool result; re-run the tool if needed]`,
    );
  });
});
...
describe('HTTP client', () => {
  it('builds an OpenRouter decisions request', () => {
